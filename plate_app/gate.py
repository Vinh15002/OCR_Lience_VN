...
class TcpRelayGate(GateController):
    ...
    def __init__(self, host: str, port: int, command: bytes = b"OPEN\n", timeout: float = 2.0, sender=None):
        self.host = host
        self.port = int(port)
...
class SerialRelayGate(GateController):
    ...
    def __init__(self, port: str, baudrate: int = 9600, command: bytes = b"OPEN\n", writer=None):
        self.port = port
...
def build_gate(config, on_change=None) -> GateController:
    ...
    if backend == "tcp" and getattr(config, "gate_host", ""):
        hardware: GateController = TcpRelayGate(config.gate_host, config.gate_port, command)
        return CompositeGate(simulated, [hardware])
    if backend == "serial" and getattr(config, "gate_serial_port", ""):
        hardware = SerialRelayGate(config.gate_serial_port, config.gate_baudrate, command)
        return CompositeGate(simulated, [hardware])
    return simulated
